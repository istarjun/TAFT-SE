...
if __name__ == '__main__':
    ...
    savefile_name = 'save/TAFT_SE_Deeplab_' + encoder + 'Coco20i_%d_%ds_%dq' %(split_number, n_shot, n_query)
    matfile_name=savefile_name + '.mat'
    savefile_name_last =savefile_name + '_last'

    # set up training
    # ------------------
    model = TAFT_SE_Deeplab(encoder=encoder, nb_class = nb_class_train,  n_shot=n_shot, device=device, coco=True)
    ...
    train_generator = coco20i_generator(img_dir='/PATH/TO/COCO2014', label_dir='/PATH/TO/COCO20i/Label', aux_label_dir='/PATH/TO/COCO/Label', n_epoch=max_iter,
