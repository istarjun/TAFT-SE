...
class TAFT_SE_Deeplab(nn.Module):
  ...
  def evaluate(self, images_1, labels_1, images_2, labels_2, images_3, labels_3):
    ...
    images = torch.Tensor(images_1).to(self.device)
    labels = torch.Tensor(labels_1).to(self.device)
    with torch.no_grad():
      ...
      dec_image_1 = F.interpolate(dec_key, size=images.size()[2:4], mode='bilinear', align_corners=True)

    images = torch.Tensor(images_2).to(self.device)
    labels = torch.Tensor(labels_2).to(self.device)
    with torch.no_grad():
      key, low_level_key = self.encoder(images)
      support_set = key[:self.nb_class * self.n_shot]
      query_set = key[self.nb_class * self.n_shot:]
      ...
      dec_key = self.phi2(r_t)
      dec_image_2 = F.interpolate(dec_key, size=images.size()[2:4], mode='bilinear', align_corners=True)



    images = torch.Tensor(images_3).to(self.device)
    labels = torch.Tensor(labels_3).to(self.device)
    with torch.no_grad():
      key, low_level_key = self.encoder(images)
      support_set = key[:self.nb_class * self.n_shot]
      query_set = key[self.nb_class * self.n_shot:]
      ...
      dec_key = self.phi2(r_t)
      dec_image_3 = F.interpolate(dec_key, size=images.size()[2:4], mode='bilinear', align_corners=True)

    images = torch.Tensor(images_2).to(self.device)
    labels = torch.Tensor(labels_2).to(self.device)

    with torch.no_grad():
      dec_image_r1 = F.interpolate(dec_image_1, size=images.shape[2:4], mode='bilinear', align_corners=True)
      dec_image_r3 = F.interpolate(dec_image_3, size=images.shape[2:4], mode='bilinear', align_corners=True)
      dec_image_r = (dec_image_r1 + dec_image_2 + dec_image_r3) / 3
      dec_image = torch.argmax(dec_image_r, dim=1, keepdim=True)
